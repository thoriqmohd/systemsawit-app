...
const storageKey = "systemsawit-premium-demo-v1";
...
export function SistemSawitApp() {
  ...
  const [selectedCollection, setSelectedCollection] = useState<string>(initialState.collections[0]?.id ?? "");
  ...
  if (!signedIn) return <AuthScreen forgot={forgot} setForgot={setForgot} setRole={setRole} setSignedIn={setSignedIn} />;
    ...
    <div className="flex min-h-screen">
      <aside className="hidden w-72 shrink-0 border-r border-white/60 bg-white/80 p-4 shadow-soft backdrop-blur-xl lg:block">
        <div className="flex items-center gap-3 rounded-2xl bg-darkField p-4 text-white shadow-glow">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-limewash">SistemSawit</p>
            <h1 className="text-lg font-semibold">PalmOps Cloud</h1>
          </div>
        </div>
        <nav className="mt-5 space-y-1">
          {visibleNav.map(({ key, label, icon: Icon }) => <button className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold ${page === key ? "bg-canopy-600 text-white shadow-soft" : "text-slate-600 hover:bg-canopy-50 hover:text-canopy-900"}`} key={key} onClick={() => setPage(key as PageKey)}><Icon size={18} />{label}</button>)}
        </nav>
      ...
      <section className="min-w-0 flex-1">
        ...
        <div className="px-4 py-5 md:px-6 lg:px-8">
          <MobileNav page={page} setPage={setPage} items={visibleNav.map((item) => ({ key: item.key as PageKey, label: item.label }))} />
    ...
    <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-6xl overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 shadow-glow backdrop-blur lg:grid-cols-[1.05fr_0.95fr]">
      <section className="relative flex flex-col justify-between p-8 md:p-10">
        ...
        <div className="relative mt-10 grid gap-3 md:grid-cols-3">
          {["GPS proof", "QR tracking", "Payment-ready"].map((item) => <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/10" key={item}><p className="text-sm font-semibold">{item}</p><p className="mt-2 text-xs leading-5 text-white/60">Built for Malaysian plantation teams.</p></div>)}
        </div>
      </section>
...
function Topbar({ role, query, setQuery, setSignedIn }: { role: Role; query: string; setQuery: (value: string) => void; setSignedIn: (value: boolean) => void }) {
  return <header className="sticky top-0 z-20 border-b border-white/60 bg-white/75 px-4 py-3 backdrop-blur-xl md:px-6 lg:px-8">
    <div className="flex items-center gap-3">
      <button className="rounded-xl border border-slate-200 bg-white p-2 lg:hidden"><Menu size={18} /></button>
      <div className="relative min-w-0 flex-1">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} />
        <input className="w-full rounded-2xl border border-slate-200 bg-white/90 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-canopy-400 focus:ring-2 focus:ring-canopy-100" placeholder="Search estate, worker, collection ID..." value={query} onChange={(event) => setQuery(event.target.value)} />
      </div>
      <button className="hidden rounded-2xl border border-slate-200 bg-white p-2.5 text-slate-600 md:block"><Bell size={18} /></button>
      <div className="hidden items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2 md:flex"><div className="flex h-8 w-8 items-center justify-center rounded-xl bg-canopy-100 text-canopy-700"><UserRound size={16} /></div><div><p className="text-sm font-semibold text-ink">{role}</p><p className="text-xs text-slate-500">Demo workspace</p></div></div>
      <button className="rounded-2xl bg-ink px-3 py-2 text-sm font-semibold text-white" onClick={() => setSignedIn(false)}>Logout</button>
    </div>
  </header>;
}

function MobileNav({ page, setPage, items }: { page: PageKey; setPage: (page: PageKey) => void; items: Array<{ key: PageKey; label: string }> }) {
...
function DashboardPage({ state, setPage }: { state: AppState; setPage: (page: PageKey) => void }) {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Command centre" title="Smart palm operations dashboard" description="Real-time view of FFB collection, estate performance, grading status and pending payments across your Malaysian plantation network." actionLabel="Add Collection" onAction={() => setPage("collections")} />
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <StatCard label="FFB collected today" value={`${number.format(collectedToday / 1000)} t`} helper="Across active collection centres" icon={Leaf} tone="green" />
      <StatCard label="Monthly yield" value={`${number.format(monthlyYield / 1000)} t`} helper="+13.4% vs last month" icon={BarChartIcon} tone="blue" />
      <StatCard label="Active workers" value={String(state.workers.filter((w) => w.status === "Active").length)} helper="Attendance monitored by estate" icon={Users} tone="purple" />
      <StatCard label="Payment summary" value={currency.format(paymentTotal)} helper={`${pending} collections need action`} icon={BadgeDollarSign} tone="amber" />
    </div>
...
function CollectionsPage({ state, persist, selectedId, setSelectedId, activeCollection, updateCollection }: { state: AppState; persist: (state: AppState) => void; selectedId: string; setSelectedId: (id: string) => void; activeCollection?: CollectionRecord; updateCollection: (record: CollectionRecord) => void }) {
  ...
  return <div className="space-y-6"><PageHeader eyebrow="FFB collection" title="Create and track collection records" description="Step-by-step flow from Draft to Collected, Delivered, Graded, Approved and Paid with QR-style collection IDs." />
    <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]"><Panel><h3 className="text-lg font-semibold text-ink">New collection</h3><form className="mt-4 grid gap-3" onSubmit={submit}><Select label="Estate" value={form.estateId} onChange={(v) => setForm({...form, estateId:v})} options={state.estates.map((e) => [e.id,e.name])}/><Input label="Block" value={form.block} onChange={(v) => setForm({...form, block:v})}/><Select label="Worker/team" value={form.workerId} onChange={(v) => setForm({...form, workerId:v})} options={state.workers.map((w) => [w.id,w.name])}/><Select label="Lorry" value={form.lorryId} onChange={(v) => setForm({...form, lorryId:v})} options={state.lorries.map((l) => [l.id,l.plateNo])}/><Input label="Collection centre" value={form.centre} onChange={(v) => setForm({...form, centre:v})}/><div className="grid gap-3 sm:grid-cols-3"><Input label="Bunch count" type="number" value={form.bunchCount} onChange={(v) => setForm({...form, bunchCount:v})}/><Input label="Estimated kg" type="number" value={form.estimatedWeight} onChange={(v) => setForm({...form, estimatedWeight:v})}/><Input label="Actual kg" type="number" value={form.actualWeight} onChange={(v) => setForm({...form, actualWeight:v})}/></div><button className="rounded-2xl bg-canopy-600 px-4 py-3 font-semibold text-white">Create collection</button></form></Panel><Panel>{activeCollection ? <CollectionDetail state={state} record={activeCollection} updateCollection={updateCollection} /> : <EmptyState title="No collection selected" description="Create or select a collection to view its workflow timeline." />}</Panel></div>
    <Panel><DataTable headers={["ID", "Estate", "Worker", "Centre", "Payable", "Status", "Action"]} rows={state.collections.map((item) => [<button className="font-semibold text-canopy-700" key="id" onClick={() => setSelectedId(item.id)}>{item.id}</button>, estateName(state,item.estateId), workerName(state,item.workerId), item.centre, `${number.format(getPayableWeight(item))} kg`, <StatusBadge key="s" status={item.status}/>, <button className="rounded-xl border border-slate-200 px-3 py-1.5 text-xs font-semibold" key="a" onClick={() => updateCollection(nextCollectionStatus(item))}>Advance</button>])}/></Panel>
  </div>;
}
...
function WeighbridgePage({ state, updateCollection }: { state: AppState; updateCollection: (record: CollectionRecord) => void }) {
  ...
  return <div className="space-y-6"><PageHeader eyebrow="Weighbridge and grading" title="Incoming delivery grading" description="Record gross weight, tare weight, deduction percentage, moisture, dirt and grade quality to calculate payable weight." />
    <Panel><DataTable headers={["ID", "Lorry", "Gross", "Tare", "Net", "Grade", "Receipt"]} rows={incoming.map((item) => [item.id, lorryName(state,item.lorryId), `${number.format(item.grossWeight)} kg`, `${number.format(item.tareWeight)} kg`, `${number.format(getNetWeight(item))} kg`, <StatusBadge key="g" status={item.grade}/>, <button className="rounded-xl bg-canopy-600 px-3 py-1.5 text-xs font-semibold text-white" key="r" onClick={() => updateCollection({ ...item, grossWeight: item.grossWeight || item.actualWeight + 6200, tareWeight: item.tareWeight || 6200, actualWeight: item.actualWeight || item.estimatedWeight, grade: item.grade === "Pending" ? "B" : item.grade, deductionPercent: item.deductionPercent || 2, moisturePercent: item.moisturePercent || 1, dirtPercent: item.dirtPercent || 0.5, status: "Graded", rate: item.grade === "A" ? gradeRates.A : gradeRates.B })}>Generate receipt</button>])}/></Panel>
  </div>;
}

function TransportPage({ state }: { state: AppState }) {
  return <div className="space-y-6"><PageHeader eyebrow="Transport" title="Lorry and trip operations" description="Monitor driver assignment, delivery status, fuel cost and trip performance." actionLabel="Add Trip" /><div className="grid gap-5 md:grid-cols-3">{state.lorries.map((lorry) => <Panel key={lorry.id}><div className="flex items-start justify-between"><div><h3 className="text-lg font-semibold text-ink">{lorry.plateNo}</h3><p className="text-sm text-slate-500">{lorry.driver}</p></div><StatusBadge status={lorry.status}/></div><div className="mt-5 grid grid-cols-3 gap-3"><Metric label="Trips" value={String(lorry.trips)} /><Metric label="Delivered" value={`${number.format(lorry.deliveredKg/1000)} t`} /><Metric label="Fuel" value={currency.format(lorry.fuelCost)} /></div></Panel>)}</div></div>;
}

function PaymentsPage({ state, updateCollection }: { state: AppState; updateCollection: (record: CollectionRecord) => void }) {
  return <div className="space-y-6"><PageHeader eyebrow="Finance" title="Payment vouchers and billing" description="Calculate payment from payable weight, grade rate, deductions and commission. Mark approved collections as paid." />
    <Panel><DataTable headers={["Voucher", "Worker", "Grade", "Payable", "Rate", "Amount", "Payment", "Action"]} rows={state.collections.map((item) => [item.id.replace("FFB", "PAY"), workerName(state,item.workerId), <StatusBadge key="g" status={item.grade}/>, `${number.format(getPayableWeight(item))} kg`, currency.format(item.rate), currency.format(getPaymentAmount(item)), <StatusBadge key="p" status={getPaymentStatus(item)}/>, item.status !== "Paid" ? <button className="rounded-xl bg-ink px-3 py-1.5 text-xs font-semibold text-white" key="pay" onClick={() => updateCollection({ ...item, status: "Paid" })}>Mark as paid</button> : <ReceiptText key="done" className="text-canopy-600" size={18}/>])}/></Panel>
  </div>;
}
...
function ReportsPage({ state }: { state: AppState }) {
  ...
  return <div className="space-y-6"><PageHeader eyebrow="Reports" title="Analytics and export centre" description="Filter daily collection, monthly yield, productivity, estate performance and payment reports. Export buttons are prepared for PDF and Excel." />
    <Panel><div className="mb-4 flex flex-wrap gap-3"><Select label="Status" value={status} onChange={setStatus} options={["All", ...statusSteps.map(String)].map((item) => [item,item])}/><button className="self-end rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold">Export PDF</button><button className="self-end rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold">Export Excel</button></div><DataTable headers={["Date", "Collection", "Estate", "Worker", "Weight", "Amount", "Status"]} rows={rows.map((item) => [shortDate(item.date), item.id, estateName(state,item.estateId), workerName(state,item.workerId), `${number.format(getNetWeight(item))} kg`, currency.format(getPaymentAmount(item)), <StatusBadge key="s" status={item.status}/>])}/></Panel>
  </div>;
}

function SettingsPage({ role }: { role: Role }) {
  return <div className="space-y-6"><PageHeader eyebrow="Admin settings" title="Company, permissions and rules" description="Manage users, role permissions, FFB price/rate settings, grading rules, company profile and notification preferences." />
    <div className="grid gap-5 lg:grid-cols-3"><Panel><ShieldCheck className="text-canopy-600"/><h3 className="mt-3 font-semibold text-ink">Role permissions</h3><p className="mt-2 text-sm text-slate-500">Current role: {role}. Menus adapt based on operational responsibility.</p></Panel><Panel><BadgeDollarSign className="text-canopy-600"/><h3 className="mt-3 font-semibold text-ink">FFB rates</h3><p className="mt-2 text-sm text-slate-500">Grade A RM760/t, B RM710/t, C RM650/t. Reject not payable.</p></Panel><Panel><Bell className="text-canopy-600"/><h3 className="mt-3 font-semibold text-ink">Notifications</h3><p className="mt-2 text-sm text-slate-500">Alerts for low productivity, pending grading and unpaid collections.</p></Panel></div>
  </div>;
}

function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-sm font-semibold text-ink">{value}</p></div>;
}
